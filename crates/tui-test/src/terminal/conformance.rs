...
pub struct Divergences {
    ...
    /// The backend does not implement OSC 52 clipboard storage or queries.
    pub clipboard_unsupported: bool,
}
...
macro_rules! emulator_conformance_tests {
    ...
    ($make:expr, $divergences:expr) => {
        ...
        fn conformance_clipboard_support_is_explicit() {
            ...
            if CONFORMANCE_DIVERGENCES.clipboard_unsupported {
                assert!(
                    clipboard.is_err(),
                ...
                assert!(
                    fault.contains("not supported"),
                    "the fault explains that clipboard access is unsupported: {fault}"
                );
            } else {
                assert_eq!(
